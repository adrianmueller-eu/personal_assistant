...
require_once __DIR__."/chat_manager.php";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $telegram = new Telegram($chat_id, $telegram_token);
    // $telegram->send_message("You said: ".$message); // Debug

    if (strpos(file_get_contents(__DIR__."/update_ids.txt"), $update_id) !== false) {
    ...
    if ($chat_id != $chat_id_admin) {
        ...
        $telegram2 = new Telegram($chat_id_admin, $telegram_token);
    ...
    // Initialize the chat manager, openai manager and command manager
    $chat_manager = new ChatManager($chat_id);
    ...
    $command_manager->add_command(array("/start", "/reset", "/r"), function($command, $args) use ($telegram, $chat_manager) {
        $chat_manager->save(array(
            "model" => "gpt-4",
            "temperature" => 0.9,
            "messages" => array(
    ...
    }, "Presets", "Generic personal assistant");

    // The command /therapist is a preset for a therapist
    $command_manager->add_command(array("/therapist", "/therapy", "/t"), function($command, $args) use ($telegram, $chat_manager, $openai) {
        $chat = array(
            "model" => "gpt-4",
            "temperature" => 0.5,
            "messages" => array(
                array("role" => "system", "content" => "You are a therapist assisting me to connect to myself and heal. Show compassion by acknowledging and validating my feelings. Your primary goal is to provide a safe, nurturing, and supportive environment for me. Your task is to help me explore my thoughts, feelings, and experiences, while guiding me towards personal growth and emotional healing. You are also familiar with Internal Family Systems (IFS) therapy and might use it implicitly to guide the process. Keep your responses very short and compact, but as helpful as possible."),
                // array("role" => "system", "content" => "Open the session with a short message. Keep it short and invite sharing."),
            )
        );
        $chat_enc = json_encode($chat);
        ...
        $initial_response = $openai->gpt($chat_enc);
        $telegram->send_message($initial_response);
        $chat = json_decode($chat_enc, false); // Somehow this seems to be necessary, won't debug now
        // // Delete the last message from the chat history, because it is only required for the initial response
        ...
        $chat_manager->save($chat);
    }, "Presets", "Therapist");

    // The command /responder writes a response to a given message
    $command_manager->add_command(array("/responder", "/re"), function($command, $args) use ($telegram, $chat_manager, $openai) {
        $chat_manager->save(array(
            "model" => "gpt-4",
            "temperature" => 0.7,
    ...
    $command_manager->add_command(array("/summarizer", "/sum"), function($command, $args) use ($telegram, $chat_manager, $openai) {
        $chat_manager->save(array(
            "model" => "gpt-4",
            "temperature" => 0.7,
    ...
    $command_manager->add_command(array("/translator", "/trans"), function($command, $args) use ($telegram, $chat_manager, $openai) {
        $chat_manager->save(array(
            "model" => "gpt-4",
            "temperature" => 0.7,
    ...
    $command_manager->add_command(array("/calendar", "/cal"), function($command, $args) use ($telegram, $chat_manager, $openai) {
        $chat_manager->save(array(
            "model" => "gpt-4",
            "temperature" => 0,
    ...
    $command_manager->add_command(array("/paper", "/research"), function($command, $args) use ($telegram, $chat_manager, $openai) {
        $chat_manager->save(array(
            "model" => "gpt-4",
            "temperature" => 0.7,
    ...
    }, "Presets", "Research paper generator");
    ...
    $command_manager->add_command(array("/model", "/m"), function($command, $args) use ($telegram, $chat_manager) {
        $chat = $chat_manager->get();
        $telegram->send_message("You are currently talking to ".($chat->model).".\nAvailable models are /gpt4 and /chatgpt.");
    }, "Settings", "Show the current model or change it");

    // Specific commands /gpt4 and /chatgpt for the respective models
    $command_manager->add_command(array("/gpt4", "/chatgpt"), function($command, $args) use ($telegram, $chat_manager) {
        $models = array(
            "/gpt4" => "gpt-4",
    ...
    }, "Settings", "Specific commands /gpt4 and /chatgpt for the respective models");

    // The command /temperature shows the current temperature and allows to change it
    $command_manager->add_command(array("/temperature", "/temp"), function($command, $args) use ($telegram, $chat_manager) {
        $chat = $chat_manager->get();
        if (is_numeric($args)) {
            $args = floatval($args);
            if ($args >= 0 && $args <= 2) {
                $chat->temperature = $args;
                $chat_manager->save($chat);
    ...
    // The command /dump outputs the chat history and model parameters
    $command_manager->add_command(array("/dump", "/d"), function($command, $args) use ($telegram, $chat_manager) {
        $chat = $chat_manager->get();
        $telegram->send_message(json_encode($chat, JSON_PRETTY_PRINT));
    }, "Chat history management", "Dump the chat history and model parameters as JSON");

    // The command /clear clears the chat history
    $command_manager->add_command(array("/clear", "/clr"), function($command, $args) use ($telegram, $chat_manager) {
        $chat = $chat_manager->get();
        $chat->messages = array();
        $chat_manager->save($chat);
    ...
    $command_manager->add_command(array("/delete", "/del"), function($command, $args) use ($telegram, $chat_manager) {
        if (is_numeric($args)) {
            $args = intval($args);
            if ($args > 0) {
                $n = $chat_manager->delete_messages($args);
                if ($n == 0) {
            ...
            $n = $chat_manager->delete_messages(1);
            if ($n == 0) {
                $telegram->send_message("There are no messages to delete.");
            } else {
    ...
    $command_manager->add_command(array("/user", "/u"), function($command, $args) use ($telegram, $chat_manager) {
        $chat_manager->add_message("user", $args);
        $telegram->send_message("Added user message to chat history.");
    }, "Chat history management", "Add a user message to the chat history");

    // The command /assistant adds an assistant message to the chat history
    $command_manager->add_command(array("/assistant", "/a"), function($command, $args) use ($telegram, $chat_manager) {
        $chat_manager->add_message("assistant", $args);
        $telegram->send_message("Added assistant message to chat history.");
    }, "Chat history management", "Add an assistant message to the chat history");

    // The command /system adds a system message to the chat history
    $command_manager->add_command(array("/system", "/s"), function($command, $args) use ($telegram, $chat_manager) {
        $chat_manager->add_message("system", $args);
        $telegram->send_message("Added system message to chat history.");
    }, "Chat history management", "Add a system message to the chat history");
    ...
    $command_manager->add_command(array("/continue", "/c"), function($command, $args) use ($telegram, $chat_manager, $openai) {
        $chat = $chat_manager->get();
        $chat = json_encode($chat);
        $response = $openai->gpt($chat);
        $chat_manager->add_message("assistant", $response);
        $telegram->send_message($response);
    }, "Misc", "Request a response from the model");

    // The command /image requests an image from the model
    // Hint: Use $openai->dalle($prompt) to request an image from DALL·E
    $command_manager->add_command(array("/image", "/img", "/i"), function($command, $args) use ($telegram, $openai) {
        if ($args == "") {
        ...
        if (filter_var($args, FILTER_VALIDATE_URL)) {
            $telegram->send_image($args);
            exit;
        }
        $image_url = $openai->dalle($args);
        if ($image_url == "") {
            $telegram->send_message("WTF-Error: Could not generate an image. Please try again later.");
            exit;
        }
        log_image($args, $image_url, $telegram->get_chat_id());
    ...
    }, "Misc", "Request an image from the model");
    ...
    if (substr($message, 0, 1) == "." || (substr($message, 0, 1) == "\\" && !(substr($message, 1, 1) == "." || substr($message, 1, 1) == "\\"))) {
        ...
        send_message("Did you mean the command /".substr($message, 1)." ? If not, escape the first character with '\\'.", $chat_id);
        exit;
    }
    ...
    $chat_enc = json_encode($chat_manager->get());
    $response = $openai->gpt($chat_enc);
    ...
    // if (substr($response_message, 0, 15) == "BEGIN:VCALENDAR") {
    //     $telegram->send_file("event.ics", $response_message);
    // } else {
    ...
    $telegram = new Telegram($chat_id_admin, $telegram_token);
